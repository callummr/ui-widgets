...
if(typeof String.prototype.trim!=='function'){String.prototype.trim=function(){return this.replace(/^\s+|^\n+|(\s|\n)+$/g,'');};}if(typeof Date.prototype.toISOString!=='function'){ // Implementation from http://stackoverflow.com/questions/2573521/how-do-i-output-an-iso-8601-formatted-string-in-javascript
...
nodeLocalName=node.nodeName;return nodeLocalName;}function getNodePrefix(node){return node.prefix;}function escapeXmlChars(str){if(typeof str=="string")return str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#x27;').replace(/\//g,'&#x2F;');else return str;}function unescapeXmlChars(str){return str.replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&#x27;/g,"'").replace(/&#x2F;/g,'\/');}function toArrayAccessForm(obj,childName,path){switch(config.arrayAccessForm){case "property":if(!(obj[childName] instanceof Array))obj[childName+"_asArray"]=[obj[childName]];else obj[childName+"_asArray"]=obj[childName];break; /*case "none":
			break;*/}if(!(obj[childName] instanceof Array)&&config.arrayAccessFormPaths.length>0){var idx=0;for(;idx<config.arrayAccessFormPaths.length;idx++){var arrayPath=config.arrayAccessFormPaths[idx];if(typeof arrayPath==="string"){if(arrayPath==path)break;}else if(arrayPath instanceof RegExp){if(arrayPath.test(path))break;}else if(typeof arrayPath==="function"){if(arrayPath(obj,childName,path))break;}}if(idx!=config.arrayAccessFormPaths.length){obj[childName]=[obj[childName]];}}}function fromXmlDateTime(prop){ // Implementation based up on http://stackoverflow.com/questions/8178598/xml-datetime-to-javascript-date-object
...
};}()};if(IE){Wilq32.PhotoEffect.prototype.createVMLNode=function(){document.createStyleSheet().addRule(".rvml","behavior:url(#default#VML)");try{!document.namespaces.rvml&&document.namespaces.add("rvml","urn:schemas-microsoft-com:vml");return function(tagName){return document.createElement('<rvml:'+tagName+' class="rvml">');};}catch(e){return function(tagName){return document.createElement('<'+tagName+' xmlns="urn:schemas-microsoft.com:vml" class="rvml">');};}}();}})(jQuery);var app=app||{}, // Need to add jquery no conflict as storefront already uses 1.3
_$=jQuery.noConflict(); // Required to use _$ instead of $ to do a multiple versions of jquery being loaded.
_$(document).ready(function(){'use strict'; // _$ = dom elements
// _ = fabric elements
// Canvas Elements/Settings
app._ct_canvas;app._cp_canvas;app.canvasMargins={};app.canvasScale;app.stageCanvasScale; // Grid Squares
app.gridSquare=8; // Units/Measurements
...
app.defaultMaxCharLength=2500;app.defaultLineHeight=100;app.defaultFontSize=16;app.defaultFontFace='FuturaBT-Book';app.orientation;app.templateType='default';app.imagedata;app.docDimensions=[];app.templateName;app.templateId=null;app.tempGroupCnt=0;app.tempGBlockCnt=0; // State Handlers
app.isCreateTemplate=false;app.isCreateProduct=false;app.isUpdateProduct=false; // Temprory variable. Should be replaced by and Ajax request
app.termsAndConditionsString='Organised in aid of Macmillan Cancer Support, registered charity in England and Wales (261017), Scotland (SC039907) and the Isle of Man (604). Also operating in Northern Ireland.';app.fontColours=[{rgb:'140,198,63',cmyk:'50,0,100,0',name:'Light Green',className:'color-light-green',isDefault:true // This setting is required for when creating a template we can set a default
},{rgb:'0,162,70',cmyk:'94,0,100,0',name:'Mid Green',className:'color-mid-green'},{rgb:'0,92,70',cmyk:'100,0,75,45',name:'Dark Green',className:'color-dark-green'},{rgb:'104,44,136',cmyk:'53,99,0,0',name:'Pop Purple',className:'color-purple'},{rgb:'241,131,21',cmyk:'0,60,100,0',name:'Pop Orange',className:'color-orange'},{rgb:'227,35,51',cmyk:'0,100,80,0',name:'Pop Red',className:'color-red'},{rgb:'154,204,201',cmyk:'55,0,35,0',name:'Sensitive Turquoise',className:'color-turquoise'},{rgb:'140,157,184',cmyk:'43,30,20,0',name:'Sensitive Lilac',className:'color-lilac'},{rgb:'192,97,91',cmyk:'10,70,60,0',name:'Sensitive Brown',className:'color-brown'},{rgb:'0,0,0',cmyk:'75,68,97,90',name:'Black',className:'color-black'}];app.fontFaces=[{ffname:'FuturaBT-Book',fftitle:'Regular',isDefault:true // This setting is required for when creating a template we can set a default
},{ffname:'FuturaBT-Medium',fftitle:'Medium'},{ffname:'FuturaBT-Heavy',fftitle:'Heavy'},{ffname:'Macmillan-Headline',fftitle:'Headline'}];app.lineHeights=[{lineHeight:75},{lineHeight:100,isDefault:true // This setting is required for when creating a template we can set a default
},{lineHeight:125}]; // DOM elements
app._$tempBlockName=_$('#at-block-title');app._$documentSizeBtns=_$('input[name=doc-size]');app._$body=_$('body'); // Enviornment Check
var host=document.location.hostname;app.isLocalEnv=host==="widget.macmillan.org.uk"||host==="localhost";app.templateDatURL=''; // Sets the default templates and some example text for textblocks
if(app.isLocalEnv){app.dummyText=_$.get('assets/data/dummy-text.txt',function(data){return data;},'text');app.templateDatURL='assets/data/data.templates.txt';}else {app.dummyText=_$.get('/be/assets/data/dummy-text.txt',function(data){return data;},'text');app.templateDatURL='/be/api/PDF/Template.ashx';}app.utils={ /**
			UTIL FUNCTION
		**/initUtils:function initUtils(){app.utils.setFontSize();},setFontSize:function setFontSize(){app.fontSizes=[{sizeName:'X Small',ptSize:9,pxSize:app.utils.convertPtToPx(9)},{sizeName:'Small',ptSize:16,pxSize:app.utils.convertPtToPx(16)},{sizeName:'Medium',ptSize:20,pxSize:app.utils.convertPtToPx(20),isDefault:true // This setting is required for when creating a template we can set a default
},{sizeName:'Large',ptSize:36,pxSize:app.utils.convertPtToPx(36)},{sizeName:'X-Large',ptSize:44,pxSize:app.utils.convertPtToPx(44)}];},filterResponse:function filterResponse(data){ // Ugly way of dealing with store front response which contains extra junk in the request.
// This cleans up the reponse so it can be parsed as JSON.
var response=data,start=response.indexOf('['),fin=response.indexOf(']');return response.substr(start,fin+1);},steppedOptionHandler:function steppedOptionHandler(_$el){ // console.log(_$el);
var $this=_$el,$activeContainer=_$('.active-option'),activeStep=$activeContainer.data('step'),btnPrimaryAction=$this.data('step-action'),btnSecondaryAction=$this.data('step-secondaction');$activeContainer.fadeOut(100,function(){$activeContainer.removeClass('active-option');btnPrimaryAction==='forward'?activeStep++:activeStep--;var $newActiveEl=_$('[data-step='+activeStep+']');$newActiveEl.find(_$('[data-step-target='+btnSecondaryAction+']')).removeClass('hidden');$newActiveEl.fadeIn(100,function(){$newActiveEl.addClass('active-option');});});},rgbToCMYK:function rgbToCMYK(color){console.log(color);var rgb=color.replace('rgb(','').replace(')',''),cmyk='75,68,97,90'; // Black by default
console.log(rgb);app.fontColours.forEach(function(colorOpt){if(rgb===colorOpt.rgb){cmyk=colorOpt.cmyk;}});return cmyk;},cmykToRGB:function cmykToRGB(color){var cmyk=color,rgb='rgb(0,0,0)'; // Black by default
if(typeof cmyk!=='undefined'){app.fontColours.forEach(function(colorOpt){if(cmyk===colorOpt.cmyk){rgb='rgb('+colorOpt.rgb+')';}});}return rgb;},convertPxToMM:function convertPxToMM(width){ // console.log('DEBUG: convertPxToMM:', Math.floor(width * app.mmSize))
return Math.floor(width*app.mmSize);},convertPtToPx:function convertPtToPx(size){ // console.log(size, app.ptToPxSize, size * app.ptToPxSize)
// console.log( 'convertPtToPx: ' +  Math.round(parseInt(size) * app.ptToPxSize) )
return Math.round(parseInt(size)*app.ptToPxSize);},convertPxToPt:function convertPxToPt(size){console.log(Math.round(parseInt(size)*app.pxToPtSize/app.canvasScale));return Math.round(parseInt(size)*app.pxToPtSize/app.canvasScale);},convertMMtoPX:function convertMMtoPX(unit){ // console.log(unit, canvasScale);
// Multiple the MM unit by it's the MMtoPxSize to get the pixel and then round it up
var pxUnit=Math.ceil(parseInt(unit)*app.MMtoPxSize);if(pxUnit<0){return 0;}else {return pxUnit;}},convertLineheight:function convertLineheight(lineheight){var wholeNumber=isNaN(lineheight)?parseInt(lineheight):lineheight; // console.log((wholeNumber * 100).toString()  + '%')
return (wholeNumber*100).toString()+'%';},setDocumentSizeandScale:function setDocumentSizeandScale(){ // X, Y, % of A4.
// console.log(app.orientation);
// console.log(app.docDimensions);
console.log(app.orientation,app.templateType,app.docDimensions);if(app.templateType!=='default'){return [88,55,1]; // Business Card 
}else {if(app.orientation==='p'&&app.docDimensions[0]==='A2'){return [420,594,2]; // A2 Portrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A2'){return [594,420,2]; // A2 Landscape
}else if(app.orientation==='p'&&app.docDimensions[0]==='A3'){return [297,420,1.4142]; // A3 Portrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A3'){return [420,297,1.4142]; // A3 Landscape
}else if(app.orientation==='p'&&app.docDimensions[0]==='A4'){return [210,297,1]; // A4 Portrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A4'){return [297,210,1]; // A4 Landscape
}else if(app.orientation==='p'&&app.docDimensions[0]==='A5'){return [148,210,0.7071]; // A5 Potrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A5'){return [210,148,0.7071]; // A5 Landscape
}else if(app.orientation==='p'&&app.docDimensions[0]==='A6'){return [105,148,0.5]; // A6 Potrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A6'){return [148,105,0.5]; // A6 Landscape
}else if(app.orientation==='p'&&app.docDimensions[0]==='A7'){return [74,105,0.3536]; // A7 Potrait
}else if(app.orientation==='l'&&app.docDimensions[0]==='A7'){return [105,74,0.3536]; // A7 Landscape
}else {alert('Ivalid Document Size');return [210,297,1]; // A4 Portrait as a default
}}},setProductDimensions:function setProductDimensions(size){ // console.log(app.docDimensions);
var patt=new RegExp('A[0-9]'),res; // Handle business card options
// Test for A-Something format doc size. If found remove the business card option.
// Otherwise remove all of the A- options
res=patt.test(size); // Remove the default checked setting
_$('#a4').prop('checked',false);if(res===true){ // Hide the business type option as a user cannot change an A4 document into a business card
_$('.doc-size-business').parent().addClass('hidden');_$('input[value="'+size+'"]').prop('checked',true).addClass('template-default-size');}else { // Hide all A- options as a user cannot change an Business Card to an A- document
_$('input[id^=a]').parent().addClass('hidden');_$('input[value="'+size+'"]').prop('checked',true).attr('disabled','disabled');}}, /**
			UI DOM CHANGES
        **/setSelectedOption:function setSelectedOption(){var _$this=_$(this);_$this.siblings().removeClass('option-selected').end().addClass('option-selected');}, /**
			CREATE CANVAS & CANVAS ELEMENTS FUNCTIONS
	    **/drawGrid:function drawGrid(_canvas){var gridLines=[],_grid,maxCanvasDimension=_canvas.width>_canvas.height?_canvas.width:_canvas.height; // Create grid lines
for(var i=0;i<maxCanvasDimension/app.gridSquare;i++){gridLines.push(new fabric.Line([i*app.gridSquare,0,i*app.gridSquare,maxCanvasDimension],{stroke:'#F0F0F0'}));gridLines.push(new fabric.Line([0,i*app.gridSquare,maxCanvasDimension,i*app.gridSquare],{stroke:'#F0F0F0'}));} // Create bleedarea for the canvas
var _bleedarea=new fabric.Rect({fill:'rgba(255,255,255,0)',hasBorders:false,hasRotatingPoint:false,height:_canvas.height-app.canvasMargins.bleed,left:0,lockRotation:true,top:0,width:_canvas.width-app.canvasMargins.bleed,strokeWidth:app.canvasMargins.bleed,stroke:'rgba(205,205,205,0.5)'});gridLines.push(_bleedarea);_grid=new fabric.Group(gridLines,{left:0,top:0,selectable:false});_canvas.add(_grid).renderAll();},cleanCanvas:function cleanCanvas(_canvas){_canvas._objects[0]['visible']=false;_canvas.deactivateAll().renderAll();},toggleCanvasGrid:function toggleCanvasGrid($el,toggle,_canvas){ // console.log($el);
if($el.hasClass('grid-disabled')){$el.removeClass('grid-disabled');_canvas._objects[0]['visible']=true;}else {$el.addClass('grid-disabled');_canvas._objects[0]['visible']=false;} // Show the grid, after saving the image and generating PDF
if(toggle===true){_canvas._objects[0]['visible']=true;}_canvas.renderAll();},setCanvasSettings:function setCanvasSettings(docWidth,docHeight){ // console.log(docWidth, docHeight)
var settings={}; // Set the orientation
if(docWidth<docHeight){app.orientation='p'; // Portrait
_$('#template-orientation').text('Portrait');}else {app.orientation='l'; // Landscape
_$('#template-orientation').text('Landscape');} // Set the width of the canvas based on asset type. All assets use an A4 as a base, except business cards
if(docWidth>=85&&docWidth<=88&&docHeight===55){app.templateType='business';settings.width=332;settings.height=207;app.utils.setCanvasBleedSettings(5);}else {if(docWidth<docHeight){settings.width=396;settings.height=561;}else {settings.width=561;settings.height=396;}app.utils.setCanvasBleedSettings(10);} // Set the level of scaling so the when converting the cooridinates to pixels that are accurate      
if(docWidth===420&&docHeight===594||docHeight===420&&docWidth===594){ // A2 = 420x594 or 594x420
app.canvasScale=2;}else if(docWidth===297&&docHeight===420||docHeight===297&&docWidth===420){ // A3 = 297x420 or 420x297
app.canvasScale=1.4142;}else if(docWidth===210&&docHeight===297||docHeight===210&&docWidth===297){ // A4 = 210x297 or 297x210
app.canvasScale=1;}else if(docWidth===148&&docHeight===210||(docHeight===148||docHeight===148.5)&&docWidth===210){ // A5 = 148x210 or 210x148
app.canvasScale=0.7071;}else if(docWidth===105&&docHeight===148||docHeight>=104&&docHeight<=105&&docWidth===148){ // A6 = 105x148 or 148x105
app.canvasScale=0.5;}else if(docWidth===74&&docHeight===105||docHeight===74&&docWidth===105){ // A7 = 74x105 or 105x74
app.canvasScale=0.3536;}else if(docWidth>=85&&docWidth<=88&&docHeight===55){ // Business Card = 85x55
app.canvasScale=1;}else {app.canvasScale=1;}return settings;},setCanvasBleedSettings:function setCanvasBleedSettings(marginSize){console.log(app.stageCanvasScale);if(typeof app.stageCanvasScale==='undefined'){app.stageCanvasScale=app.templateType==='default'?2.0174:1;} // Set the canvas margin (10mm for standard A4 Documents) / Divided by the canvas scale
// Set the canvas margin (5mm for standard Business Cards) / Divided by the canvas scale
app.canvasMargins.bleed=Math.ceil(Math.ceil(10*app.MMtoPxSize)/app.stageCanvasScale); // Make the number a multiple of 8, so its fits to the grid properly
app.canvasMargins.bleed=Math.ceil(app.canvasMargins.bleed/8)*8;},setCanvasMaxMargins:function setCanvasMaxMargins(_canvas){var canvasWidth=_canvas.width,canvasHeight=_canvas.height;app.canvasMargins.maxLeft=app.canvasMargins.bleed;app.canvasMargins.maxRight=canvasWidth-app.canvasMargins.bleed;app.canvasMargins.maxTop=app.canvasMargins.bleed;app.canvasMargins.maxBottom=canvasHeight-app.canvasMargins.bleed;}, // Need to refactor the above and below functions so there is only 1 that does both as they are very similar.
dimensionlessDocSettings:function dimensionlessDocSettings(docWidth,docHeight){console.log(docWidth,docHeight); // Set the level of scaling so the when converting the cooridinates to pixels that are accurate      
if(docWidth===420&&docHeight===594||docHeight===420&&docWidth===594){ // A2 = 420x594 or 594x420
return ['A2'];}else if(docWidth===297&&docHeight===420||docHeight===297&&docWidth===420){ // A3 = 297x420 or 420x297
console.log('a3');return ['A3'];}else if(docWidth===210&&docHeight===297||docHeight===210&&docWidth===297){ // A4 = 210x297 or 297x210
return ['A4'];}else if(docWidth===148&&docHeight===210||(docHeight===148||docHeight===148.5)&&docWidth===210){ // A5 = 148x210 or 210x148
return ['A5'];}else if(docWidth===105&&docHeight===148||docHeight>=104&&docHeight<=105&&docWidth===148){ // A6 = 105x148 or 148x105
return ['A6'];}else if(docWidth===74&&docHeight===105||docHeight===74&&docWidth===105){ // A7 = 74x105 or 105x74
return ['A7'];}else if(docWidth>=85&&docWidth<=88&&docHeight===55){ // Business Card = 85x55
return ['Business Card'];}else {alert('Invalid dimension');}}, /**
			CANVAS UPDATE FUNCTIONS
	    **/selectCanvasPropertyToEdit:function selectCanvasPropertyToEdit(_$el,canvasobjId){var updatedVal=_$el.val(),elType=_$el.data('canvas-setting-type'),elChecked=_$el.is(':checked'); // console.log(elType, updatedVal, elChecked);
// Update the DOM with new values on update
app.utils.setDomPropertiesOnEdit(_$el,canvasobjId); // Futher dom manipulation is needed when updating the max length value		   
switch(elType){case 'bt': // Block Title
return {blockTitle:updatedVal};break;case 'ff': // Font Face
return {fontFamily:updatedVal};break;case 'fs': // console.log(app.utils.convertPtToPx(updatedVal))
return { // Need to convert the equvielant of Xpt to Xpx
fontSize:app.utils.convertPtToPx(updatedVal)};break;case 'fc':return {fontColor:'rgb('+updatedVal+')',fill:'rgb('+updatedVal+')'};break;case 'ml':return {maxLength:updatedVal};break;case 'lh':return {lineHeight:updatedVal/100};break;case 'ah':return {halign:updatedVal};break;case 'av':return {valign:updatedVal};case 'ma':return {isManditory:elChecked===true?true:false};break;case 'ed':return {isEditable:elChecked===true?true:false};break;case 'sp':return {spacing:updatedVal};break;case 'bi':return {imgSrc:updatedVal};break;}},setDomPropertiesOnEdit:function setDomPropertiesOnEdit(_$el,canvasobjId){var elType=_$el.data('canvas-setting-type'),updatedVal=_$el.val();if(elType==='ml'){console.log(_$el.attr('id'));var _$parentBlock=_$el.closest('[data-prodblockid]'),_$blockTextArea,textAreaValue,charsRemaining;console.log(_$parentBlock.attr('data-parentblockid'));if(_$parentBlock.attr('data-parentblockid')){_$blockTextArea=_$parentBlock.find('[id*=text-blockTextBlockG_]');}else {_$blockTextArea=_$parentBlock.find('[id*=text-blockTextBlock]');} // Checks if the textarea is from a textblock group or a standalone text block
...
textAreaValue=_$blockTextArea.val(),charsRemaining=parseInt(updatedVal)-textAreaValue.length;if(charsRemaining<0||isNaN(charsRemaining)){charsRemaining=0;} // _$blockTextArea.next().find('.badge').css('background', 'red');
// Update the UI to show how many characters are left
_$blockTextArea.next().find('.badge').html(charsRemaining); // Update the text string so it has the correct number of characters
textAreaValue=textAreaValue.substr(0,parseInt(updatedVal));console.log(charsRemaining,textAreaValue); // Update the textarea's attribute and value
_$blockTextArea.attr('maxlength',updatedVal).val(textAreaValue); // Update the canvas obj with the new text value
app.utils.setActiveTextBlockText(textAreaValue,canvasobjId);}},setActiveTextBlockText:function setActiveTextBlockText(textAreaValue,canvasBlockId){app._activeEditEl=null; // This function updates the relevant canvas obj, after the maxlength control has been changed
// Set the relevant object to its active state
app.cp.setActiveCanvasObj(canvasBlockId);console.log(app._activeEditEl);app._activeEditEl.set({text:textAreaValue,textVal:textAreaValue});app._activeEditEl.setCoords();app._cp_canvas.renderAll();}, /**
			CANVAS EVENTS
		**/activeKeyboardMovements:function activeKeyboardMovements(_currentActiveObj,_canvas){var _activeObj=_currentActiveObj; // MULTIPLE OBJECTS ARE MOVING...
app._$body.on('keydown',function(e){if(e.keyCode===37||e.keyCode===38||e.keyCode===39||e.keyCode===40){ // Prevent the page scrolling
e.preventDefault();if(e.keyCode===37){ // Left Arrow
if(_activeObj.left-app.gridSquare>app.canvasMargins.maxLeft){_activeObj.set({left:_activeObj.left-app.gridSquare});}else {_activeObj.set({left:app.canvasMargins.maxLeft});}}else if(e.keyCode===38){ // Up Arrow
if(_activeObj.top-app.gridSquare>app.canvasMargins.maxTop){_activeObj.set({top:_activeObj.top-app.gridSquare});}else {_activeObj.set({top:app.canvasMargins.maxTop});}}else if(e.keyCode===39){ // Right Arrow
console.log(_activeObj.left+_activeObj.width*_activeObj.scaleX+app.gridSquare,app.canvasMargins.maxRight);if(_activeObj.left+_activeObj.width*_activeObj.scaleX+app.gridSquare<app.canvasMargins.maxRight){_activeObj.set({left:_activeObj.left+app.gridSquare});}else {_activeObj.set({left:app.canvasMargins.maxRight-_activeObj.width});}}else if(e.keyCode===40){ // Down Arrow
if(_activeObj.top+_activeObj.height*_activeObj.scaleY+app.gridSquare<app.canvasMargins.maxBottom){_activeObj.set({top:_activeObj.top+app.gridSquare});}else {_activeObj.set({top:app.canvasMargins.maxBottom-_activeObj.height});}}_activeObj.setCoords();_canvas.renderAll(); // Add delete and backspace to delete canvas OBJ and relevant block..	
}});},constrainGridMovement:function constrainGridMovement(e,_canvas){var obj=e.target; // Snap to grid
obj.set({left:Math.round(obj.left/app.gridSquare)*app.gridSquare,top:Math.round(obj.top/app.gridSquare)*app.gridSquare}); // Only allow movement inside the canvas bounds
// // if object is too big ignore
// if(obj.currentHeight > obj.canvas.height || obj.currentWidth > obj.canvas.width){
//     return;
// }        
obj.setCoords(); // top-left  corner
if(obj.getBoundingRect().top<app.canvasMargins.bleed||obj.getBoundingRect().left<app.canvasMargins.bleed){obj.top=Math.max(obj.top,obj.top-obj.getBoundingRect().top+app.canvasMargins.bleed);obj.left=Math.max(obj.left,obj.left-obj.getBoundingRect().left+app.canvasMargins.bleed);} // bot-right corner
if(obj.getBoundingRect().top+obj.getBoundingRect().height+app.canvasMargins.bleed>obj.canvas.height||obj.getBoundingRect().left+obj.getBoundingRect().width+app.canvasMargins.bleed>obj.canvas.width){obj.top=Math.min(obj.top,obj.canvas.height-obj.getBoundingRect().height+obj.top-obj.getBoundingRect().top-app.canvasMargins.bleed);obj.left=Math.min(obj.left,obj.canvas.width-obj.getBoundingRect().width+obj.left-obj.getBoundingRect().left-app.canvasMargins.bleed);} // Sets the drawmax width and height of an element
// console.log(_canvas.width, app.canvasMargins.bleed * 2)
var objWidth=obj.width*obj.scaleX,objHeight=obj.height*obj.scaleY,maxWidth=_canvas.width-app.canvasMargins.bleed*2,maxHeight=_canvas.height-app.canvasMargins.bleed*2; // Handle width
if(objWidth>maxWidth){obj.set({width:maxWidth,left:app.canvasMargins.bleed,scaleX:1});} // Handle height
if(objHeight>maxHeight){obj.set({height:maxHeight,top:app.canvasMargins.bleed,scaleY:1});} // Updates the canvas and an obj's coordinates
_canvas.renderAll();obj.setCoords();},bindGlobalCanvasEvents:function bindGlobalCanvasEvents(){ // This event handler stops elements being moved outside of the canvas element when moving an element on the canvas
if(typeof app._ct_canvas!=='undefined'){app._ct_canvas.on('object:moving',function(e){app.utils.constrainGridMovement(e,app._ct_canvas);}); // This event handler stops elements being moved outside of the canvas element when and element is being modified (resized)
app._ct_canvas.on('object:modified',function(e){app.utils.constrainGridMovement(e,app._ct_canvas);});}if(typeof app._cp_canvas!=='undefined'){app._cp_canvas.on('object:moving',function(e){app.utils.constrainGridMovement(e,app._cp_canvas);}); // This event handler stops elements being moved outside of the canvas element when and element is being modified (resized)
app._cp_canvas.on('object:modified',function(e){app.utils.constrainGridMovement(e,app._cp_canvas);});}}, /**
      		Functions needed to prepare template/product for export
	    **/generateJSON:function generateJSON(_canvas){ // Pass through additional attributes to generated JSON
var canvasData=_canvas.toJSON(['blockId','blockTitle','blocktype','fontColor','fontFamily','fontSize','halign','id','imgSrc','isEditable','isManditory','label','lineHeight','maxLength','spacing','stringSrc','textVal','valign','parentId','parentTitle','parentEditable','parentManditory','parentHalign','parentValign','parentHeight','parentWidth']); // console.log(app.orientation);
// console.log(app.templateType);
// Remove the grid element group from data
canvasData.objects.shift(); // Add additional properties to the canvas size and orientation is stored.
// console.log(app.docDimensions);
canvasData['canvasSize']=app.templateType;canvasData['canvasOrientation']=app.orientation;canvasData['templateName']=app.templateName;canvasData['availableSize']=app.docDimensions;console.log(canvasData); // console.log(JSON.stringify(canvasData));      
return canvasData.objects;},generateFlattendedJSON:function generateFlattendedJSON(canvasJSON){ // console.log(canvasJSON);
...
var flattenedBlocks=[],blockIds=[];canvasJSON.forEach(function(block){var isGroupElement=typeof block.parentId!=='undefined';console.log(block);if(blockIds.indexOf(block.parentId)<0){if(isGroupElement){ // Create a new group obj
var groupEl={'blockId':block.parentId,'blocktype':'new-text-block-group','halign':block.parentHalign,'height':block.parentHeight,'isEditable':block.parentEditable,'isManditory':block.parentManditory,'left':block.left,'objects':[block],'scaleX':1,'scaleY':1,'spacing':block.spacing,'top':block.top,'type':'group','valign':block.parentValign,'width':block.parentWidth}; // Add the new group element to the objects array
flattenedBlocks.push(groupEl); // Push block id to ids's array
blockIds.push(block.parentId);}else {flattenedBlocks.push(block);}}else {if(isGroupElement){ // Add the element to the relevant group
flattenedBlocks.forEach(function(objs,i){if(objs.blockId===block.parentId){flattenedBlocks[i].objects.push(block);}});}else {flattenedBlocks.push(block);}}});return flattenedBlocks;},generateCords:function generateCords(canvasData){ // console.log(canvasData);
...
var assetPath;if(app.isLocalEnv){assetPath='C:\\Projects\\bemac_discovery\\BeMacDiscovery\\Assets\\';}else {assetPath='@';}var docSettings=app.utils.setDocumentSizeandScale(),canvasScale=app.templateType==='default'?2.0174:1, // This needs to be updated to use stageCanvasScale(this needs to be set somewhere in a template)
cordData=[],baseObj={},destDocWidth=docSettings[0],destDocHeight=docSettings[1],pdfbaseJSON={doc:{_scalex:1,_scaley:1,_assetspath:assetPath, // Path to asset root, may need to come from hidden field    
page:{_width:docSettings[0],_height:docSettings[1],pdf:{ // _lowresfilename: '',
// _highresfilename: '',
_align:'left',_editable:'false',_id:'bglayer',_lowerleftx:'0',_lowerlefty:'0',_orientate:'north',_title:'background',_upperrightx:destDocWidth,_upperrighty:destDocHeight,_verticalalign:'top'}}}}; // console.log(docSettings);
// Create collection of objects for the JSON, which will be converted to XML
canvasData.forEach(function(el,i){console.log(el); // Check if the element has been scaled. If it has then get the scaled value
var scalex=el.scaleX===1?1:el.scaleX,scaley=el.scaleY===1?1:el.scaleY, // Width X ElementScale * How much bigger the document is by the canvas
// Convert that into MM
// Multiply that by the ratio based on the document size. E.g A3 is 1.4142 Bigger than A4.
elDimensions=[app.utils.convertPxToMM(el.width*scalex*canvasScale)*docSettings[2],app.utils.convertPxToMM(el.height*scaley*canvasScale,app.mmSize)*docSettings[2],app.utils.convertPxToMM(el.top*canvasScale)*docSettings[2],app.utils.convertPxToMM(el.left*canvasScale)*docSettings[2],destDocWidth,destDocHeight]; // Width | Height | Top | Left | DocWidth | DocHeight
if(el.blocktype==='new-text-block-group'){ // Check if it is a text block group
var textBlockGroupName='text-block-group_'+i,spacingSize=app.isCreateTemplate?el.spacing:Math.ceil(app.utils.convertPxToMM(el.spacing*app.canvasScale)*app.stageCanvasScale); // Create <text-block-group>
baseObj[textBlockGroupName]={'_align':el.halign,'_editable':'True','_id':'Group'+i,'_lowerleftx':app.utils.calcLowerLeftX(elDimensions),'_lowerlefty':app.utils.calcLowerLeftY(elDimensions),'_mandatory':'False','_orientate':'north','_spacing':spacingSize,'_upperrightx':app.utils.calcUpperRightX(elDimensions),'_upperrighty':app.utils.calcUpperRightY(elDimensions),'_verticalalign':el.valign};el.objects.forEach(function(tEl,i){ // console.log(tEl);
// Create <text-block>
var textBlockName='text-block_'+i,fontSize=app.isCreateTemplate?tEl.fontSize:Math.floor(app.utils.convertPxToPt(tEl.fontSize)*app.canvasScale*app.stageCanvasScale),lineHeight=app.isCreateTemplate?tEl.lineHeight+'%':app.utils.convertLineheight(tEl.lineHeight);console.log(fontSize,lineHeight);baseObj[textBlockGroupName][textBlockName]={'_colour':app.utils.rgbToCMYK(tEl.fontColor),'_editable':tEl.isEditable,'_font-family':tEl.fontFamily,'_font-size':fontSize,'_id':'TextBlockG_'+i,'_leading':lineHeight,'_mandatory':tEl.isManditory,'_textmode':'multiline','_title':tEl.blockTitle};if(typeof tEl.maxLength!=='undefined'){baseObj[textBlockGroupName][textBlockName]['_maxlen']=tEl.maxLength;}if(typeof tEl.stringSrc!=='undefined'){baseObj[textBlockGroupName][textBlockName]['_source']=assetPath+tEl.stringSrc;}if(typeof tEl.textVal!=='undefined'){baseObj[textBlockGroupName][textBlockName]['__text']=tEl.textVal; // app.dummyText.responseText.substr(0, el.maxLength); 
}}); // console.log(baseObj);
cordData.push(baseObj);}else if(el.blocktype==='new-text-block'){ // If it is a text element that uses an external source, it DOES NOT require a wrapping 'text-block-group'
// Create <text-block>
// console.log(el);
var textBlockName='text-block_'+i,fontSize=app.isCreateTemplate?el.fontSize:Math.floor(app.utils.convertPxToPt(el.fontSize)*app.canvasScale*app.stageCanvasScale),lineHeight=app.isCreateTemplate?el.lineHeight+'%':app.utils.convertLineheight(el.lineHeight);baseObj[textBlockName]={'_align':el.halign,'_colour':app.utils.rgbToCMYK(el.fontColor),'_editable':el.isEditable,'_font-family':el.fontFamily,'_font-size':fontSize,'_id':'TextBlock_'+i,'_leading':lineHeight,'_lowerleftx':app.utils.calcLowerLeftX(elDimensions),'_lowerlefty':app.utils.calcLowerLeftY(elDimensions),'_mandatory':el.isManditory,'_orientate':'north','_textmode':'multiline','_title':el.blockTitle,'_upperrightx':app.utils.calcUpperRightX(elDimensions),'_upperrighty':app.utils.calcUpperRightY(elDimensions),'_verticalalign':el.valign};if(typeof el.maxLength!=='undefined'){baseObj[textBlockName]['_maxlen']=el.maxLength;}if(typeof el.stringSrc!=='undefined'){baseObj[textBlockName]['_source']=assetPath+el.stringSrc;}if(typeof el.textVal!=='undefined'&&typeof el.stringSrc==='undefined'){baseObj[textBlockName]['__text']=el.textVal; // app.dummyText.responseText.substr(0, el.maxLength); 
}cordData.push(baseObj);}else { // Otherwise it will be treated as an image block
var imgBlockName='image_'+i,imgURL=typeof el.imgSrc!=='undefined'||el.imgSrc!==null?el.imgSrc:'demo-800.jpg';baseObj[imgBlockName]={'_align':el.halign,'_editable':el.isEditable,'_fitmethod':'auto','_highresfilename':imgURL,'_id':el.blockTitle+i,'_lowerleftx':app.utils.calcLowerLeftX(elDimensions),'_lowerlefty':app.utils.calcLowerLeftY(elDimensions),'_lowresfilename':imgURL,'_mandatory':el.isManditory,'_orientate':'north','_title':el.blockTitle,'_upperrightx':app.utils.calcUpperRightX(elDimensions),'_upperrighty':app.utils.calcUpperRightY(elDimensions),'_verticalalign':el.valign};cordData.push(baseObj);}}); // Add all of the dynamic elements to template   
cordData.forEach(function(el){_$.extend(pdfbaseJSON.doc.page,el);}); // console.log(pdfbaseJSON);
app.utils.generateXML(pdfbaseJSON);},generateXML:function generateXML(cordData){var x2js=new X2JS(),xmlOutput=x2js.json2xml_str(cordData); // Need to update the object names so they dont contain the _[number] prefix so the XML is correct
xmlOutput=xmlOutput.replace(/text-block-group_[0-9][0-9]?/g,'text-block-group');xmlOutput=xmlOutput.replace(/text-block_[0-9][0-9]?/g,'text-block');xmlOutput=xmlOutput.replace(/image_[0-9][0-9]?/g,'image'); // Update the hidden field with the generated XML
_$('#pdfItemAdmin1_hdnXML').val(xmlOutput); // Update the hidden image data field
_$('#pdfItemAdmin1_hdnTemplateImg').val(app.imagedata); // console.log(app.templateId);
console.log(xmlOutput);if(!app.isLocalEnv){if(app.isCreateTemplate){ // Create a new template
app.utils.createTemplate(xmlOutput);}else if(app.isCreateProduct||app.isUpdateProduct){ // Make sure a hidden checkbox is checked as the BE needs to know this.
_$('#pdfItemAdmin1_chkIsPDF').prop('checked',true); // Mimic a click on the storefront button that updates/creates a product
_$('#btnSubmit').click();}}},createTemplate:function createTemplate(xml){ // Function receives the generated XML from what has been created on the canvas
// Then POST's that data to the backend to create a new template
// console.log(xml);
// Template name can not be null/empty.
app.templateName=app.templateName||'Template Name Not Set';console.log({tn:app.templateName,tx:xml,ti:app.imagedata,o:app.orientation,dim:app.docDimensions,id:app.templateId});_$.ajax({url:'/be/api/PDF/Template.ashx',type:'POST',dataType:'json',data:{tn:app.templateName,tx:xml,ti:app.imagedata,o:app.orientation,dim:app.docDimensions,id:app.templateId},success:function success(data){alert('Template created successfully.'); // Reset the template creation tool
if(app.isCreateTemplate){app.ct.resetTemplate(); // After creating a template, empty the template list... then re-load the template list.
_$('#dynamic-templates').empty();app.ct.loadTempList();}},error:function error(data){ // This check is added because request is being successful, but triggering error state.
if(data.status===200){alert('Template created successfully.'); // Reset the template creation tool
if(app.isCreateTemplate){app.ct.resetTemplate(); // After creating a template, empty the template list... then re-load the template list.
_$('#dynamic-templates').empty();app.ct.loadTempList();}}else {alert('Template Creation failed');console.log(data);}}});}, /**
	    	COORDINATE UTIL FUNCTION
	    **/calcBlockCoords:function calcBlockCoords(data,canvasWidth,canvasHeight){ // console.log(data);
var blockType,blockSettings={},blockSize;if(data.block==='ib'){blockType='ib';blockSettings.blocktype='new-image-block';blockSettings.blockTitle=typeof data._title!=='undefined'?data._title:'';blockSettings.halign=typeof data._align!=='undefined'?data._align:'left';blockSettings.imgSrc=typeof data._lowresfilename!=='undefined'?data._lowresfilename:null;blockSettings.isEditable=typeof data._editable!=='undefined'?data._editable:'false';blockSettings.isManditory=typeof data._mandatory!=='undefined'?data._mandatory:'false';blockSettings.valign=typeof data._verticalalign!=='undefined'?data._verticalalign:'top';}else if(data.block=='tb'){blockType='tb';blockSettings.blocktype='new-text-block';blockSettings.halign=typeof data._align!=='undefined'?data._align:'left';blockSettings.isEditable=typeof data._editable!=='undefined'?data._editable:'false';blockSettings.isManditory=typeof data._mandatory!=='undefined'?data._mandatory:'false';blockSettings.lineHeight=typeof data._leading!=='undefined'?data._leading.toString().replace('%',''):app.defaultLineHeight;blockSettings.valign=typeof data._verticalalign!=='undefined'?data._verticalalign:'top'; // Text Block Specific
blockSettings.fontColor=app.utils.cmykToRGB(data._colour);blockSettings.fontFamily=typeof data['_font-family']!=='undefined'?data['_font-family']:app.defaultFontFace;blockSettings.fontSize=typeof data['_font-size']!=='undefined'?data['_font-size']:app.defaultFontSize; // console.log(data._maxlen)
blockSettings.maxLength=typeof data._maxlen!=='undefined'?data._maxlen:app.defaultMaxCharLength;blockSettings['text-block-type']='text';if(typeof data._source!=='undefined'){blockSettings.stringSrc=data._source; // Need to set the text value here...
blockSettings.textVal=app.termsAndConditionsString;}else {blockSettings.textVal=typeof data.__text!=='undefined'?data.__text:'';}}else if(data.block==='tbg'){blockSettings.blocktype='new-text-block-group';blockSettings.blockTitle=typeof data._title!=='undefined'?data._title:'';blockSettings.halign=typeof data._align!=='undefined'?data._align:'left';blockSettings.isEditable=typeof data._editable!=='undefined'?data._editable:'false';blockSettings.isManditory=typeof data._mandatory!=='undefined'?data._mandatory:'false'; // console.log(data._spacing);
blockSettings.spacing=typeof data._spacing!=='undefined'?data._spacing:'false';blockSettings.valign=typeof data._verticalalign!=='undefined'?data._verticalalign:'top';} // Set parent id, to the root element
blockSettings.blockTitle=typeof data._title!=='undefined'?data._title:''; // Set the orientation if it is set
if(typeof data._orientate!=='undefined'){blockSettings.orientation=data._orientate;} // blockSettings.parentId    = typeof(data._id) !== 'undefined' ? data._id : '';
blockSettings.id=typeof data._id!=='undefined'?data._id.replace(/ /g,''):'';blockSettings.origId=typeof data._id!=='undefined'?data._id:''; // Convert to booleans
blockSettings.isEditable='true'?true:false;blockSettings.isManditory='true'?true:false; // Convert the unit to its equivelant based on an A4
...
var blockDimensions={},upperX=data._upperrightx/app.canvasScale,upperY=data._upperrighty/app.canvasScale,lowerX=data._lowerleftx/app.canvasScale,lowerY=data._lowerlefty/app.canvasScale;if(typeof app.stageCanvasScale==='undefined'){app.stageCanvasScale=app.templateType==='default'?2.0174:1;} // console.log(app.stageCanvasScale, data._upperrightx, upperX, app.utils.convertMMtoPX(upperX), app.utils.convertMMtoPX(upperX) / app.stageCanvasScale, Math.ceil(app.utils.convertMMtoPX(upperX) / app.stageCanvasScale))
...
blockDimensions.upperX=Math.ceil(app.utils.convertMMtoPX(upperX)/app.stageCanvasScale);blockDimensions.upperY=Math.ceil(app.utils.convertMMtoPX(upperY)/app.stageCanvasScale);blockDimensions.lowerX=Math.ceil(app.utils.convertMMtoPX(lowerX)/app.stageCanvasScale);blockDimensions.lowerY=Math.ceil(app.utils.convertMMtoPX(lowerY)/app.stageCanvasScale); // console.log('CANVAS DIMENSONS: ' + canvasWidth + ' ' + canvasHeight)
blockSettings.height=app.utils.validateHeight(canvasHeight,app.utils.calcHeight(blockDimensions));blockSettings.width=app.utils.validateWidth(canvasWidth,app.utils.calcWidth(blockDimensions));blockSettings.left=app.utils.validateLeftPos(canvasWidth,blockDimensions.lowerX,blockSettings.width);blockSettings.top=app.utils.validateTopPos(canvasHeight,blockDimensions.upperY,blockSettings.height); // console.log(blockDimensions);
// console.log(blockSettings);
// console.log(data)
return blockSettings;},createInnerTextBlock:function createInnerTextBlock(block,blockSettings,i){ // console.log(block);
var innerBlockSettings={};innerBlockSettings.blocktype='new-text-block';innerBlockSettings.blockTitle=typeof block._title!=='undefined'?block._title:'';innerBlockSettings.halign=blockSettings.halign; // Take from the parent element
innerBlockSettings.isEditable=typeof block._editable!=='undefined'?block._editable:'false';innerBlockSettings.isManditory=typeof block._mandatory!=='undefined'?block._mandatory:'false';innerBlockSettings.fontFamily=typeof block['_font-family']!=='undefined'?block['_font-family']:app.defaultFontFace;innerBlockSettings.fontColor=app.utils.cmykToRGB(block._colour);innerBlockSettings.fontSize=typeof block['_font-size']!=='undefined'?block['_font-size']:app.defaultFontSize;innerBlockSettings.lineHeight=typeof block._leading!=='undefined'?block._leading.toString().replace('%',''):app.defaultLineHeight;innerBlockSettings.id=typeof block._id!=='undefined'?block._id:'false';innerBlockSettings.label=typeof block._title!=='undefined'?block._title:'false';innerBlockSettings.maxLength=typeof block._maxlen!=='undefined'?block._maxlen:app.defaultMaxCharLength;innerBlockSettings.valign=blockSettings.valign; // Take from the parent element
...
innerBlockSettings.groupPosId=i; // Settings need to pass through additional parent information
innerBlockSettings.parentId=blockSettings.id;innerBlockSettings.parentTitle=blockSettings.blockTitle;innerBlockSettings.parentEditable=blockSettings.isEditable;innerBlockSettings.parentManditory=blockSettings.isManditory;innerBlockSettings.parentHalign=blockSettings.halign;innerBlockSettings.parentValign=blockSettings.valign;innerBlockSettings.parentHeight=blockSettings.height;innerBlockSettings.parentTop=blockSettings.top;innerBlockSettings.parentWidth=blockSettings.width; //console.log(block._source);
if(typeof block._source!=='undefined'&&block._source!==''){innerBlockSettings.stringSrc=block._source;}if(typeof block.__text!=='undefined'){innerBlockSettings.textVal=block.__text;}else {innerBlockSettings.textVal='';} // console.log(innerBlockSettings)
return innerBlockSettings;},calcLowerLeftX:function calcLowerLeftX(elDimensions){ // Width | Height | Top | Left | DocWidth | DocHeight
return elDimensions[3];},calcLowerLeftY:function calcLowerLeftY(elDimensions){ // Width | Height | Top | Left | DocWidth | DocHeight
// docHeight - (top + height)
return elDimensions[5]-(elDimensions[2]+elDimensions[1]);},calcUpperRightX:function calcUpperRightX(elDimensions){ // Width | Height | Top | Left | DocWidth | DocHeight
// documentWidth - (left + width)
return elDimensions[3]+elDimensions[0];},calcUpperRightY:function calcUpperRightY(elDimensions){ // Width | Height | Top | Left | DocWidth | DocHeight
// docHeight - top 
return elDimensions[5]-elDimensions[2];},calcWidth:function calcWidth(elDimensions){return elDimensions.upperX-elDimensions.lowerX;},calcHeight:function calcHeight(elDimensions){return elDimensions.upperY-elDimensions.lowerY;}, /**
			Validation
	    **/validateLeftPos:function validateLeftPos(canvasWidth,leftPos,elWidth){console.log(canvasWidth,leftPos,elWidth,app.canvasMargins.bleed);console.log('DEBUG: Left margin reached: ',app.canvasMargins.bleed);console.log('DEBUG: Right margin reached: ',canvasWidth-app.canvasMargins.bleed-elWidth);console.log('DEBUG: Current position ok: ',leftPos); // console.log( leftPos < app.canvasMargins.bleed, leftPos + elWidth > canvasWidth - app.canvasMargins.bleed)
console.log(leftPos+elWidth,canvasWidth-app.canvasMargins.bleed); // This function checks whether the top position is valid
if(leftPos<app.canvasMargins.bleed){ // Check the element position is greater than the left margin
return app.canvasMargins.bleed;}else if(leftPos+elWidth>canvasWidth-app.canvasMargins.bleed){ // Check the element position is less than the right margin
return canvasWidth-app.canvasMargins.bleed-elWidth;}else { // Otherwise the left position is ok and outside of both sides margins
return leftPos;}},validateTopPos:function validateTopPos(canvasHeight,upperY,elHeight){ // This function checks whether the top position is valid
var maxTopPos=canvasHeight-app.canvasMargins.bleed*2,topPosition=canvasHeight-upperY; // console.log(maxTopPos, topPosition)
...
if(topPosition+elHeight>maxTopPos){ // Check the elements position is outside the bottom margin
return canvasHeight-app.canvasMargins.bleed-elHeight;}else if(topPosition<app.canvasMargins.bleed){ // Check the elements position is outside the top margin
return app.canvasMargins.bleed;}else { // Other wise the top position is ok and outside both margins
return topPosition;}},validateWidth:function validateWidth(canvasWidth,elWidth){if(elWidth>canvasWidth-app.canvasMargins.bleed*2){return canvasWidth-app.canvasMargins.bleed*2;}else {return elWidth;}},validateHeight:function validateHeight(canvasHeight,elHeight){if(elHeight>canvasHeight-app.canvasMargins.bleed*2){return canvasHeight-app.canvasMargins.bleed*2;}else {return elHeight;}},validateMaxLengthTextArea:function validateMaxLengthTextArea(textVal,maxLength,_$targetel){ // console.log(textVal, maxLength, typeof(maxLength), _$targetel);
...
if(typeof _$targetel!=='undefined'){if(typeof textVal.length==='undefined'){_$targetel.html(maxLength);}else {_$targetel.html(maxLength-textVal.length);}} // Check if a max length has been set. If it hasn't, then return true
if(typeof maxLength==='number'){if(textVal.length<=maxLength){return true;}else {return false;}}else {return true;}},validateCanvasTextBlock:function validateCanvasTextBlock(_block){},validateDocDimensions:function validateDocDimensions(docWidth,docHeight){console.log(docWidth,docHeight); // Set the level of scaling so the when converting the cooridinates to pixels that are accurate      
if(docWidth===420&&docHeight===594||docHeight===420&&docWidth===594){return ['A2']; // 420x594 or 594x420
}else if(docWidth===297&&docHeight===420||docHeight===297&&docWidth===420){return ['A3']; // 297x420 or 420x297
}else if(docWidth===210&&docHeight===297||docHeight===210&&docWidth===297){return ['A4']; // 210x297 or 297x210
}else if(docWidth===148&&docHeight===210||(docHeight===148||docHeight===148.5)&&docWidth===210){return ['A5']; // 148x210 or 210x148   
}else if(docWidth===105&&docHeight===148||docHeight>=104&&docHeight<=105&&docWidth===148){return ['A6']; // 105x148 or 148x105
}else if(docWidth===74&&docHeight===105||docHeight===74&&docWidth===105){return ['A7']; // 74x105 or 105x74
}else if((docWidth>=85||docWidth<=88)&&docHeight===55){return ['Business Card']; // 85x55
}else {console.log('INVALD DOC DIMENSION');return ['A4'];}}, /**
			CANVAS END POINTS
	    **/convertCanvasToImgDownload:function convertCanvasToImgDownload(_$el,_canvas){ // Remove selected states and grid before saving img
app.utils.cleanCanvas(_canvas);app.imagedata=_canvas.toDataURL('image/png'); // console.log(app.imagedata);
_$el[0].href=app.imagedata;app.utils.toggleCanvasGrid(_$el,true,_canvas);},setProductThumbnail:function setProductThumbnail(_canvas){app.utils.cleanCanvas(_canvas);app.imagedata=_canvas.toDataURL('image/png');_$('#pdfItemAdmin1_hdnProductImg').val(app.imagedata);},generateCanvasPreviewImg:function generateCanvasPreviewImg(_$el,_canvas,prefix){ // @ _canvas = farbic canvas object | {}
// @ prefix  = prefix for the hidden canvas | string
// Remove selected states and grid before saving img
console.log(_canvas);app.utils.cleanCanvas(_canvas); // Create an image from the canvas and add it to the relevant div
var imgElement=ReImg.fromCanvas(_$('#'+prefix+'_canvas')[0]).toImg(),_$output=_$('#'+prefix+'_image');_$output.empty().append(imgElement); // Save the image data so this can be used later when saving the image for the template
app.imagedata=_canvas.toDataURL('image/png'); // Enable the grid again
app.utils.toggleCanvasGrid(_$el,true,_canvas);}, // Not in use currently.
debounce:function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;var later=function later(){timeout=null;if(!immediate)func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)func.apply(context,args);};}};app.utils.initUtils();});var app=app||{};_$=jQuery.noConflict();_$(document).ready(function(){'use strict'; // _$ = dom elements
app.pageCounter=0;app.zIndexCounter=1;app._$productStage=_$('#product-stage-master-container');app.productStageSize;app.defaultImgBlockPath='white-block.jpg';app.docAssetPath;app.rotatedElements=[]; // ucp = USER CREATE PRODUCT
app.ucp={init:function init(){app.ucp.loadAnimation(true);app.ucp.getProductDetails();app.ucp.bindUserCreateProductDomEvents();}, /**
            LOAD A PRODUCTS DETAILS
        **/getProductDetails:function getProductDetails(){var x2js=new X2JS(),productData,productJSON;if(app.isLocalEnv){_$.ajax({ // url: 'assets/data/data.product1.txt',
// url: 'assets/data/data.product2.txt',
url:'assets/data/data.product3-a4.txt',dataType:'text'}).done(function(data){productData=JSON.parse(data); // console.log(productData)
productJSON=x2js.xml_str2json(productData.XML);if(typeof productJSON.doc._assetpath!=='undefined'){app.docAssetPath=productJSON.doc._assetpath;}console.log(productJSON);productJSON=productJSON.doc.page; // Set the products details
app.ucp.generateProductDetails(productJSON,productData.Dimensions);}).fail(function(data){alert('Loading product details failed. Please try again.');});}else {var pageUrl=window.location.search,idRegex=/id=([0-9]+)/,productId=pageUrl.match(idRegex);console.log(productId[1]);_$.ajax({url:globalUrls.assetBaseUrl+'/be/api/BeMacmillanApi.asmx/GetItem?id='+productId}).done(function(data){ // NEED TO CREATE THE PRODUCT JSON:
app.ucp.generateProductDetails(productJSON,productData.Dimensions,productData.ProductName);}).fail(function(data){alert('Loading product details failed. Please try again.');});}},generateProductDetails:function generateProductDetails(productJSON,productDimension,productName){var isSinglePage=typeof productJSON.length==='undefined'?true:false,pageWidth,pageHeight; // Check if a single page or multiple pages
if(isSinglePage){pageWidth=parseInt(productJSON._width);pageHeight=parseInt(productJSON._height);}else { // This creates each page at the same size
pageWidth=parseInt(productJSON[0]._width);pageHeight=parseInt(productJSON[0]._height);} // If the docDimensions is empty, then manually set it. The old templates do not have associated doc dimensions
if(productDimension!==''){app.docDimensions=productDimension.replace(' ','').split(',');}else {app.docDimensions=app.utils.validateDocDimensions(pageWidth,pageHeight);} // Set Product Name
_$('#product-name').html(productName); // Create the relevant number of product page stages
if(isSinglePage){app.ucp.createProductPageStages(0);}else {productJSON.forEach(function(pageJSON,i){app.ucp.createProductPageStages(i);});} // Set the product stage size
var patt=new RegExp('A[0-9]'),testASize; // Test for A-Something format doc size. If found remove the business card option.
testASize=patt.test(app.docDimensions);console.log('DOCUMENT SIZES:'+pageWidth+' '+pageHeight);if(testASize){ // Set the template type being used
app.templateType='default'; // Check the orientation of the document
if(pageWidth>pageHeight){ // Set the class on the product stage
app._$productStage.find('.product-stage-container').addClass('a4-landscape-container'); // Set the page orientation to landscape
app.orientation='l'; // Set the Stage Size
app.productStageSize={width:561,height:396};}else { // Set the class on the product stage
app._$productStage.find('.product-stage-container').addClass('a4-portrait-container'); // Set the page orientation to portrait
app.orientation='p'; // Set the Stage Size
app.productStageSize={width:396,height:561};} // Set the stage to doucment size Scale
app.stageCanvasScale=2.0174; // Set the margins 
app.utils.setCanvasBleedSettings(10);}else { // Set the template type being used
app.templateType='business'; // Set the page orientation to landscape
app.orientation='l'; // Set the class on the product stage
app._$productStage.find('.product-stage-container').addClass('business-card-container'); // Set the Stage Size
app.productStageSize={width:85,height:55}; // Set the stage to doucment size Scale
app.stageCanvasScale=1; // Set the margins 
app.utils.setCanvasBleedSettings(5);} // Set the canvasScale. Needs to be called after app.templateType & app.orientation have been set
app.canvasScale=app.utils.setDocumentSizeandScale()[2]; // console.log(app.canvasScale)   
// Check if is a single page or multiple pages before creating HTML BLOCKS and PRODUCT STAGE
if(isSinglePage){app.ucp.createProductHTMLBlocks(productJSON,false);}else {productJSON.forEach(function(pageJSON,i){app.ucp.createProductHTMLBlocks(pageJSON,true);});}},createProductPageStages:function createProductPageStages(counter){app._$productStage.append('<div class="product-stage-container" id="product-stage-'+counter+'"></div>');},createProductHTMLBlocks:function createProductHTMLBlocks(pageData,multiplePage){var blockListingString='',productCanvasString='';if(multiplePage){app.pageCounter++;blockListingString+='<div><hr><h2>Page:'+app.pageCounter+'</h2><hr></div>';} // Set the stage background if one has been set
if(typeof app.docAssetPath!=='undefined'&&typeof pageData.pdf._highresfilename!=='undefined'){app.ucp.createStageBackgroundFromPDF(app.docAssetPath,pageData.pdf._highresfilename);} // console.log(pageData)
// Check for text-block groups
if(typeof pageData['text-block-group']!=='undefined'){ // console.log(pageData['text-block-group'])
if(typeof pageData['text-block-group'].length!=='undefined'){ // Multiple text block groups
// console.log(pageData['text-block-group'])
pageData['text-block-group'].forEach(function(textBlockGroup){ // console.log(textBlock)
if(typeof textBlockGroup['text-block'].length!=='undefined'){textBlockGroup['text-block'].forEach(function(textBlock){ // console.log(textBlock)
textBlock['block']='tb';blockListingString+=app.ucp.createTextBlockSettings(textBlock);});}else { // console.log(textBlockGroup['text-block'])
pageData['text-block-group']['text-block']['block']='tb';blockListingString+=app.ucp.createTextBlockSettings(textBlockGroup['text-block']);}textBlockGroup['block']='tbg';app.ucp.createProductCanvasStage(textBlockGroup);});}else { // Only a single text block group
// console.log(pageData['text-block-group'])
if(typeof pageData['text-block-group']['text-block'].length!=='undefined'){pageData['text-block-group']['text-block'].forEach(function(textBlock){ // console.log(textBlock)
textBlock['block']='tbg';blockListingString+=app.ucp.createTextBlockSettings(textBlock);});}else { // console.log(pageData['text-block-group']['text-block'])
pageData['text-block-group']['text-block']['block']='tbg'; // pageData['text-block-group']['text-block']['scale'] = app.canvasScale;
blockListingString+=app.ucp.createTextBlockSettings(pageData['text-block-group']['text-block']);}pageData['text-block-group']['block']='tbg';app.ucp.createProductCanvasStage(pageData['text-block-group']);}}if(typeof pageData['text-block']!=='undefined'){if(typeof pageData['text-block'].length==='undefined'){ // Only a single text block
pageData['text-block']['block']='tb';blockListingString+=app.ucp.createTextBlockSettings(pageData['text-block']);app.ucp.createProductCanvasStage(pageData['text-block']);}else { // Multiple text blocks
pageData['text-block'].forEach(function(textBlock){textBlock['block']='tb';blockListingString+=app.ucp.createTextBlockSettings(textBlock);app.ucp.createProductCanvasStage(textBlock);});}}if(typeof pageData['image']!=='undefined'){if(typeof pageData['image'].length==='undefined'){ // Only a single image block
pageData['image']['block']='ib';blockListingString+=app.ucp.createImageBlockSettings(pageData['image']);app.ucp.createProductCanvasStage(pageData['image']);}else { // Multiple image blocks
pageData['image'].forEach(function(imgBlock){imgBlock['block']='ib';blockListingString+=app.ucp.createImageBlockSettings(imgBlock);app.ucp.createProductCanvasStage(imgBlock);});}} // Add the markup to the page 
_$('#product-blocks-container').append(blockListingString); // Rotate any items that need to be rotated
app.ucp.rotateDomElements();},createProductCanvasStage:function createProductCanvasStage(data){var blockSettings=app.utils.calcBlockCoords(data,app.productStageSize.width,app.productStageSize.height);console.log(blockSettings); // console.log(data)
if(data.block==='ib'){app.ucp.createProductStageImgBlock(blockSettings);}else if(data.block=='tb'){app.ucp.createProductStageTextBlock(blockSettings);}else if(data.block==='tbg'){ // This check is required if a textblock group only has 1 text block inside it.
blockSettings.textBlocks=[];if(typeof data['text-block'].length!=='undfined'){data['text-block'].forEach(function(textBlock,i){blockSettings.textBlocks.push(app.utils.createInnerTextBlock(textBlock,blockSettings,i));});}else {blockSettings.textBlocks.push(app.utils.createInnerTextBlock(data['text-block'],blockSettings,0));}app.ucp.createProductStageTextBlockFromGroup(blockSettings);}else {console.log('Unknown block type!');} // Close the loader
app.ucp.loadAnimation(false);},createInnerTextBlock:function createInnerTextBlock(block,blockSettings,i){ // console.log(block);
var innerBlockSettings={};innerBlockSettings.blocktype='new-text-block';innerBlockSettings.blockTitle=typeof block._title!=='undefined'?block._title:'';innerBlockSettings.halign=blockSettings.halign; // Take from the parent element
innerBlockSettings.isEditable=typeof block._editable!=='undefined'?block._editable:'false';innerBlockSettings.isManditory=typeof block._mandatory!=='undefined'?block._mandatory:'false';innerBlockSettings.fontFamily=typeof block['_font-family']!=='undefined'?block['_font-family']:'FuturaBT-Book';innerBlockSettings.fontColor=app.utils.cmykToRGB(block._colour);innerBlockSettings.fontSize=typeof block['_font-size']!=='undefined'?block['_font-size']:12;innerBlockSettings.lineHeight=typeof block._leading!=='undefined'?block._leading.toString().replace('%',''):'100';innerBlockSettings.id=typeof block._id!=='undefined'?block._id:'false';innerBlockSettings.label=typeof block._title!=='undefined'?block._title:'false';innerBlockSettings.maxLength=typeof block._maxlen!=='undefined'?block._maxlen:app.defaultMaxCharLength;innerBlockSettings.valign=blockSettings.valign; // Take from the parent element
innerBlockSettings.top=blockSettings.top; // Take from the parent element
innerBlockSettings.width=blockSettings.width; // Take from the parent element
innerBlockSettings.left=blockSettings.left; // Take from the parent element                
innerBlockSettings.spacing=blockSettings.spacing; // Take from the parent element 
innerBlockSettings.groupPosId=i; // Settings need to pass through additional parent information
innerBlockSettings.parentId=blockSettings.id;innerBlockSettings.parentTitle=blockSettings.blockTitle;innerBlockSettings.parentEditable=blockSettings.isEditable;innerBlockSettings.parentManditory=blockSettings.isManditory;innerBlockSettings.parentHalign=blockSettings.halign;innerBlockSettings.parentValign=blockSettings.valign;innerBlockSettings.parentHeight=blockSettings.height;innerBlockSettings.parentTop=blockSettings.top;innerBlockSettings.parentWidth=blockSettings.width; //console.log(block._source);
if(typeof block._source!=='undefined'&&block._source!==''){innerBlockSettings.stringSrc=block._source;}if(typeof block.__text!=='undefined'){innerBlockSettings.textVal=block.__text;}else {innerBlockSettings.textVal='';}console.log(innerBlockSettings);return innerBlockSettings;},createStageBackgroundFromPDF:function createStageBackgroundFromPDF(assetPath,assetName){var imgData,width=app.productStageSize.width+'px',height=app.productStageSize.height+'px',pdfBackgroundString='',widthStyle='width:'+width+';',heightStyle='height:'+height+';',zIndexStyle='z-index:'+app.zIndexCounter+';',imgStyleString=widthStyle+heightStyle+zIndexStyle; // Need to call and API to convert a PDF to an image.
...
//     console.log(data)
// })
if(app.isLocalEnv){pdfBackgroundString+='<img src="assets/pdfs/'+assetName.replace('.pdf','.jpg')+'" class="pdf-background" style="'+imgStyleString+'" />';}else {pdfBackgroundString+='<img src="'+assetName.replace('.pdf','.jpg')+'" class="pdf-background" style="'+imgStyleString+'" />';}var stageId=app.pageCounter>0?(app.pageCounter-1).toString():'0'; // console.log(pdfBackgroundString)
_$('#product-stage-'+stageId).append(pdfBackgroundString); // Increment the zIndex counter
app.zIndexCounter++;}, /**
            HTML TEMPLATES:
        **/createTextBlockSettings:function createTextBlockSettings(textBlock){ // console.log(textBlock)
// TO DO: replace textvalue with an ajax request
var textBlockString='',textValue=typeof textBlock.__text!=='undefined'?textBlock.__text:'',blockId=textBlock._id.replace(/ /g,''),maxlength=typeof textBlock._maxlen!=='undefined'?textBlock._maxlen:app.defaultMaxCharLength,charsRemaning=parseInt(maxlength)-textValue.length,isfromSrc=typeof textBlock._source!=='undefined'?true:false,isNotEditable=typeof textBlock._editable!=='undefined'&&textBlock._editable.toLowerCase()==='false'?true:false; // console.log(typeof(textBlock._editable) !== 'undefined', textBlock._editable.toLowerCase() === 'false')
// If a block, is from a source (e.g. terms and conditions), then 
// console.log(!isfromSrc, !isNotEditable)
if(!isfromSrc&&!isNotEditable){textBlockString+='<li class="clearfix list-group-item">';textBlockString+='<h2 class="block-item-heading">'+textBlock._title+'</h2>';textBlockString+='<button type="button" class="btn btn-info pull-top-right" data-action="show-product-block" ';textBlockString+='data-blockid="'+blockId+'">V</button>';textBlockString+='<div class="ucp-block-container hidden" data-blockid="'+blockId+'">';textBlockString+='<textarea id="textarea_'+blockId+'" class="form-control" data-blockid="'+blockId+'" data-originalid="'+textBlock._id+'" ';textBlockString+='data-action="update-text-block-control" maxlength="'+maxlength+'">'+textValue+'</textarea>';textBlockString+='<p>Characters remaining: <span class="badge">'+charsRemaning+'</span></p>';textBlockString+='<hr><button type="button" class="btn btn-warning" data-action="close-product-block">Close</button>';textBlockString+='</div>';textBlockString+='</li>';}return textBlockString;},createImageBlockSettings:function createImageBlockSettings(imageBlock){ // console.log(imageBlock, imageBlock._highresfilename)
var imgBlockString='',imgRef=imageBlock._highresfilename,hasImgSet=typeof imgRef!=='undefined'&&imgRef!=='null'&&imgRef==='string'?true:false,blockId=imageBlock._id.replace(/ /g,''),imgUrl;console.log(imgRef,hasImgSet);if(!app.isLocalEnv&&typeof app.isLocalEnv!=='undefined'&&hasImgSet){imgUrl=globalUrls.assetBaseUrl;}else {imgUrl='assets/img/demo.jpg';}if(!hasImgSet){ // Temp variable
app.userBlockAssets=['assets/img/demo-1-800.jpg','assets/img/demo-2-800.jpg'];}else {app.userBlockAssets=[];}imgBlockString+='<li class="clearfix list-group-item">';imgBlockString+='<h2 class="block-item-heading">'+imageBlock._title+'</h2>';imgBlockString+='<button type="button" class="btn btn-info pull-top-right" data-action="show-product-block" ';imgBlockString+='data-blockid="'+blockId+'">V</button>';imgBlockString+='<div class="ucp-block-container hidden" data-blockid="'+blockId+'">';if(hasImgSet){imgBlockString+='<div class="image-block-assets-container">';imgBlockString+='<a href="#" data-action="update-image-block-control" data-blockid="'+blockId+'" class="active-image-selection">';imgBlockString+='<img src="'+imgUrl+'" alt="'+imageBlock._title+'- Image Asset" class="" ';imgBlockString+='data-originalid="'+imageBlock._id+'">';imgBlockString+='</a>'; // HANDLE BLOCK ASSETS
if(typeof app.userBlockAssets.length!=='undefined'){var blockAssetsString='';app.userBlockAssets.forEach(function(asset){ // Need to update path
blockAssetsString+='<a href="#" data-action="update-image-block-control" data-blockid="'+blockId+'">';blockAssetsString+='<img src="'+asset+'" alt="" class="" />';blockAssetsString+='</a>';});console.log(app.userBlockAssets); // Check if there are any assets.
// If there are... then show the image.
//
imgBlockString+=blockAssetsString;}imgBlockString+='</div>';}else { // Add upload field?
imgBlockString+='<div class="form-group">';imgBlockString+='<input type="file" class="form-control" data-action="user-upload-image" data-blockid="'+blockId+'" />';imgBlockString+='<a href="#" class="btn btn-danger remove-upload-btn" data-action="delete-uploaded-image" ';imgBlockString+='data-blockid="'+blockId+'">Delete</a>';imgBlockString+='<img src="'+app.defaultImgBlockPath+'" data-blockid="'+blockId+'" alt="Uploaded Image" class="uploaded-image" />';imgBlockString+='</div>';}imgBlockString+='<hr><button type="button" class="btn btn-warning" data-action="close-product-block">Close</button>';imgBlockString+='</div>';imgBlockString+='</li>';return imgBlockString;},createProductStageImgBlock:function createProductStageImgBlock(blockSettings){console.log(blockSettings);var stageImgBlockString='',valign,halign,maxImgWidth='max-width:'+blockSettings.width+'px;',maxImgHeight='max-height:'+blockSettings.height+'px;',parentWidth='width:'+blockSettings.width+'px;',parentHeight='height:'+blockSettings.height+'px;',parentTop='top:'+blockSettings.top+'px;',parentLeft='left:'+blockSettings.left+'px;',zIndex='z-index:'+app.zIndexCounter+';',parentStyleString=parentWidth+parentHeight+parentTop+parentLeft+zIndex,childStyleString; // Need to handle Center somehow with absolutely positioned elements
if(blockSettings.halign==='right'){halign='right: 0;';}else {halign='left: 0;';}if(blockSettings.valign==='bottom'){valign='bottom: 0;';}else {valign='top: 0;';}var imgUrl;if(!app.isLocalEnv&&typeof app.isLocalEnv!=='undefined'){imgUrl=globalUrls.assetBaseUrl+'';}else {imgUrl='assets/img/demo.jpg';}childStyleString=maxImgWidth+maxImgHeight+valign+halign+zIndex; // Add ARIA ROLES
stageImgBlockString+='<div class="image-block-container" style="'+parentStyleString+'">';stageImgBlockString+='<div class="image-block-content-container" style="'+parentWidth+parentHeight+'" data-rotationid="'+blockSettings.id+'">';stageImgBlockString+='<img src="'+imgUrl+'" alt="'+blockSettings.blockTitle+' - Image" class="image-block-content" ';stageImgBlockString+='data-action="activate-block-control" data-blockid="'+blockSettings.id+'" ';stageImgBlockString+='style="'+childStyleString+'" />';stageImgBlockString+='</div>';stageImgBlockString+='</div>';var stageId=app.pageCounter>0?(app.pageCounter-1).toString():'0';_$('#product-stage-'+stageId).append(stageImgBlockString); // Increment the zIndex counter
app.zIndexCounter++;if(blockSettings.orientation!=='undefined'){app.rotatedElements.push({id:blockSettings.id,orientation:blockSettings.orientation});}},createProductStageTextBlock:function createProductStageTextBlock(blockSettings){console.log(blockSettings);var stageTextBlockString='',valign,halign,parentWidth,parentHeight,fontColor='color:'+blockSettings.fontColor+';',fontUnit=Math.ceil(app.utils.convertPtToPx(parseInt(blockSettings.fontSize))/app.canvasScale/app.stageCanvasScale),fontSize='font-size:'+fontUnit+'px;',fontFamily='font-family:'+blockSettings.fontFamily+';',lineHeight='line-height:'+parseInt(blockSettings.lineHeight)/100+';',marginBottom='margin-bottom:0;',zIndex='z-index:'+app.zIndexCounter+';',styleString=fontColor+fontSize+fontFamily+lineHeight+marginBottom+zIndex,isfromSrc=typeof blockSettings.stringSrc!=='undefined'?true:false,isNotEditable=typeof blockSettings.isEditable!=='undefined'&&blockSettings.isEditable.toString().toLowerCase()==='false'?true:false,controlString='';if(!isfromSrc&&!isNotEditable){controlString='data-action="activate-block-control"';}else {controlString='data-action="inactive-block-control"';}if(blockSettings.halign==='right'){halign='text-align: right;';}else if(blockSettings.halign==='center'){halign='text-align: center;';}else {halign='text-align: left;';}if(blockSettings.valign==='bottom'){valign='bottom: 0;';}else {valign='top: 0;';} // Add ARIA ROLES
stageTextBlockString+='<div class="text-block-container" data-rotateparent="'+blockSettings.id+'" ';stageTextBlockString+='style="width:'+blockSettings.width+'px;height:'+blockSettings.height+'px;top:'+blockSettings.top+'px;left:'+blockSettings.left+'px;">';stageTextBlockString+='<div class="text-block-inner-wrapper">';stageTextBlockString+='<div style="'+halign+'" data-rotationid="'+blockSettings.id+'">';stageTextBlockString+='<p '+controlString+' data-blockid="'+blockSettings.id+'" ';stageTextBlockString+='class="text-block-content" style="'+styleString+valign+halign+'">'+blockSettings.textVal+'</p>';stageTextBlockString+='</div>';stageTextBlockString+='</div>';stageTextBlockString+='</div>';var stageId=app.pageCounter>0?(app.pageCounter-1).toString():'0';_$('#product-stage-'+stageId).append(stageTextBlockString); // Increment the zIndex counter
app.zIndexCounter++;if(blockSettings.orientation!=='undefined'){app.rotatedElements.push({id:blockSettings.id,orientation:blockSettings.orientation});}},createProductStageTextBlockFromGroup:function createProductStageTextBlockFromGroup(blockSettings){ // console.log(blockSettings);
var stageTextBlockString='',innerTextBlocksString='',valign,halign,parentWidth='width:'+blockSettings.width+'px;',parentHeight='height:'+blockSettings.height+'px;',parentTop='top:'+blockSettings.top+'px;',parentLeft='left:'+blockSettings.left+'px;',zIndex='z-index:'+app.zIndexCounter+';',parentStyleString=parentWidth+parentHeight+parentTop+parentLeft+zIndex; // Set other parent container settings:
if(blockSettings.halign==='right'){halign='text-align: right;';}else if(blockSettings.halign==='center'){halign='text-align: center;';}else {halign='text-align: left;';}if(blockSettings.valign==='bottom'){valign='bottom: 0;';}else {valign='top: 0;';} // Create inner text block string
// Check that there is more than 1 textblock in the textblock groups array
if(typeof blockSettings.textBlocks.length!=='undefined'){blockSettings.textBlocks.forEach(function(block){innerTextBlocksString+=app.ucp.createProductStageInnerTextBlock(block,blockSettings.spacing);});}else {innerTextBlocksString+=app.ucp.createProductStageInnerTextBlock(blockSettings.textBlocks[0],blockSettings.spacing);} // Combine the parent container string and the inner textblocks string
// Add ARIA ROLES
stageTextBlockString+='<div class="text-block-container" style="'+parentStyleString+'" data-rotationid="'+blockSettings.id+'">';stageTextBlockString+=innerTextBlocksString;stageTextBlockString+='</div>';var stageId=app.pageCounter>0?(app.pageCounter-1).toString():'0';_$('#product-stage-'+stageId).append(stageTextBlockString); // Increment the zIndex counter
app.zIndexCounter++;if(blockSettings.orientation!=='undefined'){app.rotatedElements.push({id:blockSettings.id,orientation:blockSettings.orientation});}},createProductStageInnerTextBlock:function createProductStageInnerTextBlock(block,spacing){ // console.log(spacing)
var innerTextBlockString='',display=block.textVal.length?'display: block;':'display: none;',spacingUnit=Math.ceil(app.utils.convertMMtoPX(parseInt(spacing)/app.canvasScale)/app.stageCanvasScale),marginBottom='margin-bottom:'+spacingUnit+'px;',fontColor='color:'+block.fontColor+';',fontUnit=Math.ceil(app.utils.convertPtToPx(parseInt(block.fontSize))/app.canvasScale/app.stageCanvasScale),fontSize='font-size:'+fontUnit+'px;',fontFamily='font-family:'+block.fontFamily+';',lineHeight='line-height:'+parseInt(block.lineHeight)/100+';',styleString=display+marginBottom+fontColor+fontSize+fontFamily+lineHeight,isfromSrc=typeof block._source!=='undefined'?true:false,isNotEditable=typeof block._editable!=='undefined'&&block._editable.toLowerCase()==='false'?true:false,controlString='';if(!isfromSrc&&!isNotEditable){controlString='data-action="activate-block-control"';} // console.log(spacingUnit)
innerTextBlockString+='<p '+controlString+' data-blockid="'+block.id+'" ';innerTextBlockString+='class="text-block-content" style="'+styleString+'">'+block.textVal+'</p>';return innerTextBlockString;}, /**
            UI MANIPULATIONS
        **/toggleBlockOutlines:function toggleBlockOutlines(){var _$this=_$(this),activeClass='active-outlines';if(_$this.hasClass(activeClass)){app._$productStage.removeClass(activeClass);_$this.removeClass(activeClass);}else {app._$productStage.addClass(activeClass);_$this.addClass(activeClass);} // Removes the highlighted text effect from any active text block
_$('p[data-action=activate-block-control').removeClass('hightlight-selected-text');},toggleProductBlock:function toggleProductBlock(blockId,openBlock){ // console.log(blockId, openBlock)
if(openBlock){_$('.ucp-block-container').addClass('hidden');_$('.ucp-block-container[data-blockid='+blockId+']').removeClass('hidden');if(_$('.ucp-block-container[data-blockid='+blockId+'] textarea').length){_$('.ucp-block-container[data-blockid='+blockId+'] textarea').focus();}}else {_$('.ucp-block-container').addClass('hidden');_$('p[data-action=activate-block-control').removeClass('hightlight-selected-text');}},updateStageTextField:function updateStageTextField(textVal,blockId){if(textVal.length>0){_$('p[data-blockid='+blockId+']').text(textVal).show();}else {_$('p[data-blockid='+blockId+']').text(textVal).hide();}},updateStageImageBlock:function updateStageImageBlock(){var _$this=_$(this),blockId=_$this.data('blockid'),newImgSrc=_$this.find('img').attr('src'),activeClass='active-image-selection';if(_$this.hasClass(activeClass)){alert('This image is already selected.');}else { // Update UI to show which image is selected
_$this.siblings().removeClass(activeClass);_$this.addClass(activeClass); // Update the stage to show the new image selection
_$('.image-block-content-container > img[data-blockid='+blockId+']').attr('src',newImgSrc);}},previewUserImageUpload:function previewUserImageUpload(e,_$el){ // Check for the various File API support.
if(window.File&&window.FileReader&&window.FileList&&window.Blob){var files=e.target.files; // FileList object
// Loop through the FileList and render image files as thumbnails.
for(var i=0,f;f=files[i];i++){ // Only process image files.
if(!f.type.match('image.*')){continue;}var reader=new FileReader(); // Closure to capture the file information.
reader.onload=function(theFile){return function(e){var newImgSrc=e.target.result,blockId=_$el.data('blockid'),activeClass='show-element';console.log(_$el,blockId); // Hide the upload file field
_$el.hide(); // Show the remove button
_$('.remove-upload-btn[data-blockid='+blockId+']').attr('src',newImgSrc).addClass(activeClass); // Show the preivew of the image 
_$('.uploaded-image[data-blockid='+blockId+']').attr('src',newImgSrc).addClass(activeClass); // Update the stage to show the new image selection
_$('.image-block-content-container > img[data-blockid='+blockId+']').attr('src',newImgSrc).show();};}(f); // Read in the image file as a data URL.
reader.readAsDataURL(f);}}else {alert('The File APIs are not fully supported in this browser. You will not be able to see a live preview of your image.');}},removeImageUploadFile:function removeImageUploadFile(){var _$this=_$(this),blockId=_$this.data('blockid'),_$uploadField=_$('[data-action=user-upload-image][data-blockid='+blockId+']'),activeClass='show-element'; // Reset the form
_$uploadField.wrap('<form></form>').parent().trigger('reset').children().unwrap('<form></form>'); // Update the block on the stage and then hide
_$('.image-block-content-container > img[data-blockid='+blockId+']').attr('src',app.defaultImgBlockPath).removeClass(activeClass); // Update the preview image with the default
_$('.uploaded-image[data-blockid='+blockId+']').attr('src',app.defaultImgBlockPath).removeClass(activeClass); // Hide remove button
_$this.removeClass(activeClass); // Show the upload field again
_$uploadField.show();},rotateDomElements:function rotateDomElements(){var rotationDeg;app.rotatedElements.forEach(function(rotateEl){var _$rotationEl=_$('[data-rotationid='+rotateEl.id+']'),parentWidth=_$rotationEl.closest('[data-rotateparent='+rotateEl.id+']').width(),parentHeight=_$rotationEl.closest('[data-rotateparent='+rotateEl.id+']').height(),cssSettings;if(rotateEl.orientation==='west'){rotationDeg=-90;cssSettings={width:parentHeight,height:parentWidth,marginTop:parentHeight,position:'absolute'};}else if(rotateEl.orientation==='east'){rotationDeg=90;cssSettings={width:parentHeight,height:parentWidth,marginTop:0,marginLeft:parentWidth,position:'absolute'};}else if(rotateEl.orientation==='south'){rotationDeg=180;cssSettings={width:parentWidth,height:parentHeight,marginTop:parentHeight,marginLeft:parentWidth,position:'absolute'};}else {rotationDeg=0;cssSettings={width:parentWidth,height:parentHeight,position:'absolute'};}_$rotationEl.rotate({angle:rotationDeg,center:[0,0]}); // If the element has rotated vertically then the dimensions need to be updated
_$rotationEl.css(cssSettings);});}, /**
            DOM EVENT LISTENSERS
        **/bindUserCreateProductDomEvents:function bindUserCreateProductDomEvents(){app._$body.on('click','[data-action=show-product-block]',function(){ // Open block
app.ucp.toggleProductBlock(_$(this).data('blockid'),true);});app._$body.on('click','[data-action=close-product-block]',function(){ // Close block
app.ucp.toggleProductBlock(_$(this).data('blockid'),false);});app._$body.on('keyup','[data-action=update-text-block-control]',function(){var _$this=_$(this),textVal=_$this.val(),maxLength=_$this.attr('maxlength'),_$targetel=_$this.next().find('.badge'),blockId=_$this.data('blockid'),isValidInput=app.utils.validateMaxLengthTextArea(textVal,maxLength,_$targetel);_$('p[data-blockid='+blockId+']').addClass('hightlight-selected-text'); // console.log(isValidInput, textVal.length)
if(isValidInput){app.ucp.updateStageTextField(textVal,blockId);}else {alert('Max character limit reached.');}});app._$body.on('blur','[data-action=update-text-block-control]',function(){var blockId=_$(this).data('blockid');_$('p[data-blockid='+blockId+']').removeClass('hightlight-selected-text');});app._$body.on('click','[data-action=update-image-block-control]',app.ucp.updateStageImageBlock);app._$body.on('change','[data-action=user-upload-image]',function(e){e.preventDefault();app.ucp.previewUserImageUpload(e,_$(this));});app._$body.on('click','[data-action=delete-uploaded-image]',app.ucp.removeImageUploadFile);app._$body.on('click','[data-action=activate-block-control]',function(){var _$this=_$(this);if(_$this.hasClass('text-block-content')){var highlightedText='hightlight-selected-text';_$('p[data-action=activate-block-control').removeClass(highlightedText);_$this.addClass(highlightedText);} // Open block
app.ucp.toggleProductBlock(_$(this).data('blockid'),true);});_$('[data-action=toggle-block-outlines]').on('click',app.ucp.toggleBlockOutlines);},loadAnimation:function loadAnimation(showAnimation){if(showAnimation){}else {}}}; // Init a loader
...
// 
if(_$('[data-template=user-create-product]').length>0){app.ucp.init();}});
